 ...
 *
 *purpose: main.cpp is where buffer.cpp is tested. Tests 1-6 were provided, and
 *tests 7-8 were implemented by the authors listed above.
 */
...
//tests that allocating a page, and then reading it back gives the same contents
void test1()
{
...
//Similar to test 1 but for multiple write/reads
void test2()
{
...
//tests that reading an invalid page throws an exception
void test3()
{
	try
	{
...
//Check that a page that is not pinned cannot be 'unpinned'
void test4()
{
...
//test that if all pages are pinned, exception is thrown when allocation attempt is made
void test5()
{
	for (i = 0; i < num; i++) {
...
//Makes sure an error is thrown when an attempt to flush a pinned page is made
void test6()
{
...
//Test that dispose page removes a page correctly
void test7()
{
	for(i = 1; i <= num; i++)
	{
	...
	for(i = 1; i <= num; i++)
	{
		try
		{
			...
			PRINT_ERROR("ERROR :: File should not exist. disposePage should have removed file.");
		}
		catch(InvalidPageException &e)
		{
		}
...
//Extends test 6. Test that page cannot be flushed if pinned, and that it can be flushed after
//unpin page unpins that page.
void test8()
{
	for(i = 1; i <= num; i++)
	{
		bufMgr->allocPage(file1ptr, pid[i], page); //allocates all pages, so pin count will be 1
	}
	for(i = 1; i <= num; i++)
	{
		bufMgr->readPage(file1ptr, pid[i], page); //reading page will make pin count 2
	}
	...
	for(i = 1; i <= num; i++)
	{
		//reduce pin count twice, should make pin count = 0
		bufMgr->unPinPage(file1ptr, pid[i], false); 
		bufMgr->unPinPage(file1ptr, pid[i], false);
	}
	...
	{
		PRINT_ERROR("ERROR :: Page pinned for file being flushed. Page should not be pinned at this point in execution,")
	}
