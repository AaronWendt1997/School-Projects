...
export class GrCarousel extends GrObject {
  ...
  constructor(params = {}) {
    ...
    //My code
    let horse_geom = new T.BoxGeometry(0.8, 0.8, 0.8);
    let horse_mat = new T.MeshStandardMaterial({color:'blue'});
    let horse;
    let horses = [];
    ...
    for(let i = 0; i < 10; i++)
    {
      ...
      loader.load('standing-horse.obj', function(object)
      {
        horse = object;
    
        platform_group.add(horse);
        horse.translateY(0.5);
        horse.rotateY((2 * i * Math.PI) / 10);
        horse.translateX(0.8 * 3);
   ...
   // this.time = 0;
    ...
    /*  let moverate = delta*0.01;
      ...
      if(horses[0].position.y > 5)
      {
        this.up = false;
      }
      else if(horses[0].position.y < 3)
      {
        this.up = true;
      }*/
    };
