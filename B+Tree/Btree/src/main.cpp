...
void intTests()
{
  ...
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	checkPassFail(intScan(&index, 0, GTE, 4000, LT), 4000) //Quick added test to check large range starting at zero

	//these two scans go hand in hand to test that operators were handled correctly
...
void errorTests()
{
  ...
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	//This is a test to ensure that after a scan has ended, you cannot scan anymore
	std::cout << "Start scan, end Scan, try Scan again." << std::endl;
	try
	{
		...
		RecordId foo;
		index.scanNext(foo);
		std::cout << "Scan after completed scan test failed." << std::endl;
	}
	catch(ScanNotInitializedException e)
	{
		std::cout << "Scan after completed scan test passed." << std::endl;
	}
