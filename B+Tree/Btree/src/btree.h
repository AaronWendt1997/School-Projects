...
/**
 * @brief Number of slots in B+Tree leaf for INTEGER key.
 */
//                                                  sibling ptr             key               rid
...
struct LeafNodeInt{
  ...
  int level; //set level to 0 for leaf nodes
...
class BTreeIndex {
 ...
 private:
  ...
  /**
   * Index of next entry to be scanned in  leaf being scanned.
   */
	int			nextEntry;
 ...
 /**
 *Recursive function that inserts entries into the Btree and splits nodes, shifts 
 *entry on nodes around as needed to set up the insertion method
 *
 *@param key the key that will be inserted
 *@param rid the Record ID of the entry
 *@param midKey key that will hold the information about where the node was split, should splitting occur
 *@param pid Node that will be used to check for the entry
 */
 PageId findNode(const void *key, const RecordId rid, PageId pid, void *midKey);

/**
 *Used to insert the key into the correct node
 *
 *@param old Node where we will be inserting on
 *@param key The key that is going to be inserted
 *@param nId the pid or rid that is oging to be inserted
 *@param isLeaf used to determine whether or not the passed in node is a leaf
 */
 const void insertion(void *old, const void *key, void *nId, int isleaf);
